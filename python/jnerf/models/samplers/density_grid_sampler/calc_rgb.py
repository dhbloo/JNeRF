...
class CalcRgb(Function):
    ...
    def execute(self, network_output, coords_in, rays_numsteps, density_grid_mean, rays_numsteps_compacted,training_background_color):
        ...
        # coords_in n_ray_samples_per_batch x 8
        # rays_numsteps n_ray_samples_per_batch x 2 [step,base]
        # return
        ...
        rgb_output, t_output, weights_output = jt.code(
            [(self.n_rays_per_batch, 3), (self.n_rays_per_batch, self.n_rays_step+1), (self.n_rays_per_batch, self.n_rays_step)], 
        ...
        @alias(t_output,out1)
        @alias(weights_output,out2)
        @alias(rays_numsteps_compacted,in3)
        @alias(training_background_color,in4)
        cudaStream_t stream=0;
    
        cudaMemsetAsync(t_output_p, 0, t_output->size);
        cudaMemsetAsync(weights_output_p, 0, weights_output->size);
        ...
        self.weights_output = weights_output.detach()
        return rgb_output, t_output, weights_output

    def grad(self, grad_rgb, grad_t, grad_weights):
        # return
        # dloss_doutput num_element x 4
        if grad_weights is None:
            grad_weights = jt.zeros_like(self.weights_output)

        dloss_doutput = jt.code((self.num_elements, 4), 
                                self.grad_type,
                                inputs=[self.network_output, self.rays_numsteps_compacted, self.coords_in, grad_rgb, grad_weights, self.rgb_output, self.weights_output, self.density_grid_mean], 
                                cuda_header=global_headers+self.density_grad_header+'#include "calc_rgb.h"', cuda_src=f"""
        ...
        @alias(coords_in,in2)
        @alias(grad_rgb,in3)
        @alias(grad_weights,in4)
        @alias(rgb_output,in5)
        @alias(weights_output,in6)
        @alias(density_grid_mean,in7)
        ...
        ENerfActivation density_activation=ENerfActivation({self.density_activation});
        linear_kernel(
            compute_rgbs_grad<grad_t>, 
            0,
            stream,
            n_rays, 
            m_aabb,padded_output_width,
            (grad_t*)dloss_doutput_p,
            (grad_t*)network_output_p,
            (uint32_t*)rays_numsteps_p,
            PitchedPtr<NerfCoordinate>((NerfCoordinate*)coords_in_p, 1, 0, 0),
